...
from linformer import Linformer, LinformerSelfAttention, FeedForward         # 내가 추가한거
...
class RelMultiHeadAttn(nn.Module):
    def __init__(
        ...
        #self.proj_k = nn.Parameter(init_(torch.zeros(seq_len, k)))              # 추가 @@@@@@@@@@@@@@@@@@@
...
class RelPartialLearnableMultiHeadAttn(RelMultiHeadAttn):
    def __init__(self, *args, **kwargs):
        ...
        self.r_net = nn.Linear(self.d_model, self.n_head * self.d_head, bias=False)     # d_model을 n_head * d_head 로 쪼개는 건가?
    ...
    def forward(self, w, r, r_w_bias, r_r_bias, attn_mask=None, mems=None):
        ...
        if mems is not None:
            ...
            w_heads = self.qkv_net(w)       # w : 디코더 인풋
            r_head_k = self.r_net(r)            # r : positional embedding

            w_head_q, w_head_k, w_head_v = torch.chunk(w_heads, 3, dim=-1)

        klen = w_head_k.size(0)

        w_head_q = w_head_q.view(                       # w_head_q 가 Q인가봄?
            qlen, bsz, self.n_head, self.d_head         # bsz는 batchsize를 의미하는 듯.
        ...
        """
        w_head_k, r_head_k, w_head_v 의 길이를 projection을 통해 n에서 k로 변경해주면 된다.
        그렇다 하더라도 어텐션의 최종 결과물은 n x d 이므로, FF 쪽은 건드리지 않아도 된다.
        """
        k = qlen // 8
        self.proj_k = nn.Parameter(init_(torch.zeros(qlen, k)))  
        proj_seq_len = lambda args: torch.einsum('bnd,nk->bkd', *args)
        kv_projs = self.proj_k
        w_head_k, r_head_k, w_head_v = map(proj_seq_len, zip((w_head_k, r_head_k, w_head_v), kv_projs))
